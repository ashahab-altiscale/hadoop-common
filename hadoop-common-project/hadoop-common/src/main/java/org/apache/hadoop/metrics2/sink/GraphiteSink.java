...
public class GraphiteSink implements MetricsSink {
  ...
  private static final String CONTEXTS_TO_COLLECT = "contexts_to_collect";
  private Writer writer = null;
  private Socket socket = null;
  private String metricsPrefix = null;
  private List<String> contextsToCollect = null;
  ...
  public void init(SubsetConfiguration conf) {
    ...
    // Get the list of contexts to collect. A valid list of contexts should
    // use colon as separator.
    if (conf.getString(CONTEXTS_TO_COLLECT) == null)
      contextsToCollect = null;
    else
      contextsToCollect = Arrays.asList(conf.getString(
        CONTEXTS_TO_COLLECT).split(":"));
  ...
  public void putMetrics(MetricsRecord record) {
    // Only collect contexts user specified and collect all contexts in case 
    // user does not specify anything.
    if (contextsToCollect != null &&
      !contextsToCollect.contains(record.context())) return;

    StringBuffer lines = new StringBuffer();
    StringBuffer metricsPathPrefix = new StringBuffer();

    // Configure the hierarchical place to display the graph.
    metricsPathPrefix.append(metricsPrefix)
      .append(".").append(record.context())
      .append(".").append(record.name());
    ...
    int timestamp = Math.round(record.timestamp() / 1000);

    // Collect datapoints.
    for (AbstractMetric metric : record.metrics()) {
      lines.append(metricsPathPrefix.toString() + "." + metric.name())
        .append(" ").append(metric.value())
